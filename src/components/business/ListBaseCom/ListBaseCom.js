...
const ListBaseCom = (props, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    refresh: function (config) {
      listRef?.current?.scrollToOffset({offset: 600});
      onRefresh(config);
    },
    manualRefresh: function (config, callback, offset = 600) {
      listRef?.current?.scrollToOffset({offset: offset});
      onManualRefresh(config);
      callback && callback();
    },
    scrollToEnd: () => {
  ...
  const renderRefreshControl = useMemo(
    () => <ListPullRefresh refreshing={isPull} onRefresh={onRefresh} />,
    [isPull],
  );
