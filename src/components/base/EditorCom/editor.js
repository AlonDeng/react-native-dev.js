function getContentCSS() {
  ...
  <style>
      ...
      blockquote { margin: 0; padding: 0 15px; border-left: 3px solid darkorange; background: aliceblue; }
...
<script>
  ...
  var _ = (function (exports) {
      ...
      var Actions = {
          ...
          quote: { state: function() { return queryCommandValue(formatBlock) === 'blockquote'; }, result: function() { return exec(formatBlock, '<blockquote>'); }},
          ...
          foreColor: { state: function() { return ['foreColor', queryCommandValue('foreColor')]; }, result: function(color) { return exec('foreColor', color); }},
          fontSize: { state: function() { return ['fontSize' , queryCommandValue('FontSize')]; }, result: function(size) { return exec('fontSize', size); }},
          ...
          image: {
              result: function(url, style) {
                  if (url){
                      exec('insertHTML', "<br><div style='text-align: center;'><img style='"+ (style || '')+"' src='"+ url +"'/></div><br>");
                      Actions.UPDATE_HEIGHT();
                  }
              }
          },
      ...
      var init = function init(settings) {
          ...
          function handler() {
              ...
              for(var k in actionsHandler){
                  if ( Actions[k].state() ){
                    if (k === 'fontSize') {
                      activeTools.push(k + '-' + Actions[k].state()[1]);
                    } else if (k === 'foreColor') {
                      activeTools.push(k + '-' + Actions[k].state()[1]);
                    } else {
                      activeTools.push(k);
